...
from bet_recorder.watcher import WatcherConfig, run_smarkets_watcher
    ...
    json.dumps(
      {
        ...
        "transport_path": str(bundle.transport_path) if bundle.transport_path else None,
...
def record_page(
  ...
  typer.echo(json.dumps({"source": source, "run_dir": str(run_dir), "events_path": str(bundle.events_path)}))
...
def finalize_run(
  ...
  typer.echo(json.dumps({"source": source, "metadata_path": str(bundle.metadata_path)}))
